...
export function StatCard(props: Readonly<StatCardProps>) {
  ...
  const cardContent = (
    <Card className={cn('group relative overflow-hidden transition-all duration-200 hover:-translate-y-0.5 hover:shadow-lg', href && 'cursor-pointer', className)}>
      <div className="absolute inset-x-0 top-0 h-1 bg-linear-to-r from-[#0F172A] via-[#334155] to-[#D4A017]" />
      <CardHeader className="flex flex-row items-start justify-between pb-2">
        <CardTitle className="text-sm font-medium text-slate-500">{title}</CardTitle>
        <div className="flex h-11 w-11 items-center justify-center rounded-[12px] bg-[#0F172A]/10 text-[#0F172A] ring-1 ring-[#0F172A]/10">
  ...
  return (
    <Link href={href} className="block rounded-[12px] focus-visible:outline-none focus-visible:ring-4 focus-visible:ring-[#D4A017]/15">
      {cardContent}
    </Link>
  );
}
