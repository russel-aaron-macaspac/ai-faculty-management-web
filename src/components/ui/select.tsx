...
function SelectTrigger({
  ...
  return (
    <SelectPrimitive.Trigger
      ...
      data-size={size}
      className={cn(
        "flex w-full min-w-0 items-center justify-between gap-2 rounded-xl border border-slate-300 bg-white py-2.5 pr-3 pl-3.5 text-sm whitespace-nowrap shadow-sm transition-all duration-200 outline-none select-none focus-visible:border-[#D4A017] focus-visible:ring-4 focus-visible:ring-[#D4A017]/10 disabled:cursor-not-allowed disabled:opacity-60 aria-invalid:border-rose-500 aria-invalid:ring-4 aria-invalid:ring-rose-500/10 data-placeholder:text-slate-400 data-[size=default]:h-11 data-[size=sm]:h-9 data-[size=sm]:rounded-xl *:data-[slot=select-value]:line-clamp-1 *:data-[slot=select-value]:flex *:data-[slot=select-value]:items-center *:data-[slot=select-value]:gap-1.5 [&_svg]:pointer-events-none [&_svg]:shrink-0 [&_svg:not([class*='size-'])]:size-4",
        className
      )}
      {...props}
    >
...
function SelectItem(props: Readonly<SelectPrimitive.Item.Props>) {
  ...
  return (
    <SelectPrimitive.Item
      ...
      className={cn(
        "relative flex w-full cursor-default items-center gap-2 rounded-xl py-2.5 pr-9 pl-3 text-sm outline-hidden select-none focus:bg-[#D4A017]/10 focus:text-slate-900 data-disabled:pointer-events-none data-disabled:opacity-50 [&_svg]:pointer-events-none [&_svg]:shrink-0 [&_svg:not([class*='size-'])]:size-4 *:[span]:last:flex *:[span]:last:items-center *:[span]:last:gap-2",
        className
      )}
      {...rest}
    >
